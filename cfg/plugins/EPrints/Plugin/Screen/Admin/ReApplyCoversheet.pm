...
provides an interface to allow admin users to reapply a coversheet 
to selected EPrints. The plugin renders an input form that allows
users to specify one or more ids for items to be covered.
...
sub action_cover
{
	...
	foreach my $id ( @ids ) 
	{
		...
		if ( $item )
		{
			my $covered = $repo->call( "cover_eprint_docs" , $repo, $item, $plugin, 0 );
			if ( $covered )
			{
				push @covered_ids, $id;
			}
	...
	if ( scalar @not_covered_ids )
	{
		$self->{processor}->add_message(
			"warning",
			$self->html_phrase( "not_covered", 
				items => $xml->create_text_node( join(", ", @not_covered_ids ) ) ) );
	}
