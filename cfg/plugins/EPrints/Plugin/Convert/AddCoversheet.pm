...
they are LaTeX files they are converted to pdf files.
...
sub prepare_pages
{
	...
	# my $temp_dir = "/usr/local/eprints/var";
	if( !defined $temp_dir )
	{
		EPrints::DataObj::Coversheet->log( $repo, "[Convert::AddCoversheet] Failed to create dir $temp_dir" );
		return;
	}
	...
	foreach my $coversheet_page (keys %{$pages})
	{
		...
		if ($filetype eq 'ltx')
		{
			...
			{
				$repo->call( "prepare_latex_pdf", $repo, $coversheet_page, $file_path, $eprint, $doc, $temp_dir, 1);
			}
			else
			{
...
sub export
{
	...
	if ( !defined $stored_file )
	{
		...
		print STDERR "AddCoverSheet Error: $eprintid, $filename_main\n";
		return ( $doc->get_main );
	}
	# END UZH CHANGE
