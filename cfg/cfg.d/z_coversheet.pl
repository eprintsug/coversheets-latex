...
$c->{prepare_latex_pdf} = sub 
{
	...
	my $sys_return = system( $pdflatex, "-interaction=nonstopmode", "-output-directory=$temp_dir", $latex_file );
	EPrints::DataObj::Coversheet->log( $repo, "[prepare_latex_pdf] pdflatex return code [$sys_return]" ) if $sys_return;
...
$c->{cover_eprint_docs} = sub 
{
	...
	return 0 if $has_original_cover && $has_original_cover eq "TRUE";
