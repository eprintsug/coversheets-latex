...
#	 }
	...
	$rc = $?;
	$plugin->log("executing $pdftk $pdf_file ".$plugin->get_document_file($doc)." cat output $output_path; rc=$rc")
	        if ($noise);
...
sub doc_citation {
  ...
  my $cspec = $session->get_citation_spec($eprint_ds, $cit_name);
  my $creators_gf = $self->render_with_property($eprint, 'creators_name',
						'render_order' => 'gf');
  my $datestamp_long = $self->render_with_property($eprint, 'datestamp',
						   'render_res'=>'month', 'render_style'=>'long');

  # entities should be decoded here, they are re-encoded when inserting
  # the params into the template using EPrints::XML::EPC::process()
  my %data = (
    'citation' => &_xml2latex($eprint->render_citation()),
    'url' => &_latex_escape($eprint->url),
    'content' => '',
    'creators_givenfirst' => &_xml2latex($creators_gf),
    'datestamp_long' => &_xml2latex($datestamp_long),
  );
  my $repo = $session->get_repository;
  if($repo->can_call("coverpage", "get_metadata")){
	print STDERR "could call get_metadata\n";

	%data = $repo->call( ["coverpage", "get_metadata"], $repo, $self, $eprint, $doc );
  }else{
	print STDERR "couldn't call get_metadata\n";
  }

  for my $f (qw(creators_name title type)) {
    $data{$f} = &_xml2latex($eprint->render_value($f));
  }
  #my $doc = ($eprint->get_all_documents())[0];
  #if ($doc) {
  #  $data{'content'} = &_xml2latex($doc->render_value('content'));
  #}
  my %params = (item=>$eprint,
...
sub render_with_property {
  my ($self, $dataobj, $fieldname, %props) = @_;
  my $dataset = $dataobj->get_dataset();
  my $field = $dataset->field( $fieldname );
  $field = $field->clone();
  for my $k (keys %props) {
    $field->set_property($k => $props{$k});
  }
  my $value = $field->get_value($dataobj);
  return $field->render_value($dataobj->get_session(), $value, 0, 0, $dataobj);
}
...
sub validate {
  ...
  if ($self->pdf_is_encrypted($doc)) {
    ...
    push @problems, $repository->html_phrase('coverlatex:validate_encrypted',
					     fieldname => $fieldname);
  }
  return @problems;
}
...
sub pdf_is_encrypted {
  my ($self, $input) = @_;
  my $info = (ref($input) eq 'HASH')
    ? $input : $self->_pdfinfo($self->get_document_file($input));
...
sub _pdfinfo {
  ...
  my $fh;
  open($fh, '-|', @$pdfinfo, $filename)
    or $self->log("Error: Unable to run @$pdfinfo $filename: $!");
  my $line;
  my %info = ();
  while ($line = <$fh>) {
    if ($line =~ /^([^:]+?)\s*:\s*(.+)/) {
